...
pub mod comp {
    ...
    impl<'a> Edge<'a> {
        ...
        pub fn depfile(&self) -> Option<&String> {
            if let Phony::NotPhony { depfile, .. } = &self.phony {
                depfile.as_ref()
            } else {
                None
            }
        ...
        pub fn aliases(&self) -> Option<&Aliases> {
            if let Phony::Phony { aliases, .. } = &self.phony {
                Some(aliases)
            } else {
                None
            }
...
impl<'a> Parsed<'a> {
    ...
    pub fn compile(self, arena: &'a Bump) -> Compiled<'a> {
        ...
        edges
            ...
            .iter()
            .for_each(|EdgeInput { edge, input }| {
                if !edges.contains_key(input)
                    && !fs::exists::<&Path>(input.as_ref()).unwrap_or(false)
                {
        ...
        let default_target = default_target.map(|prep::Target { loc, target }| comp::Target {
            loc,
...
impl<'a> Parser<'a> {
    ...
    fn parse_line(&mut self, not_trimmed: &'a str, trimmed: &'a str) {
        ...
        let first = line
            .find(|c: char| c.is_ascii_whitespace())
            .map(|first_space| (first_space, &line[..first_space]));

        let get_loc = || -> Loc<'_> { Loc::new(self.cursor, self.file_path) };

        let new_loc = |row: usize| -> Loc<'_> { Loc::new(row, self.file_path) };

        let Some((first_space, first_token)) = first else {
            report_panic!(get_loc(), "undefined token: {line}")
        };

        let Some(second_space) = line[first_space..]
            .find(|c: char| !c.is_ascii_whitespace())
            .map(|p| p + first_space)
        else {
            return;
        ...
        match &mut self.context {
            ...
            Context::Rule {
                ...
                COMMAND => {
                    ...
                    let rule = Rule::new(command, new_loc(command_loc), *description, *depfile);
                ...
                DESCRIPTION => {
                    ...
                    if *already_inserted {
                        let rule = self.parsed.rule_mut(name);
                        rule.description =
                            Some(Template::new(description.description, description.loc));
                    } else {
                        self.context = Context::Rule {
                            name,
                            loc: *loc,
            ...
            Context::Global => {
                match first_token {
                    SUBRUSH => {
                        ...
                        let mut string =
                            unsafe { String::from_raw_parts(buf.as_mut_ptr(), 0, buf.len()) };
                    ...
                    BUILD => {
                        ...
                        let edge = if self.parsed.phonys.contains(target) {
                            ...
                            let aliases_templates = aliases
                                .iter()
                                .map(|alias| Template::new(alias, loc))
                                .collect();
                            ...
                            let (inputs_str, deps) = if let Some(or_idx) = or_idx {
                                ...
                                let deps =
                                    post_colon[or_idx + 1..].split_ascii_whitespace().collect();
                                (inputs_str, deps)
                            } else {
                                (post_colon, Vec::new())
                            };
                            ...
                            let (inputs, inputs_str, inputs_templates) =
                                if let Some(rule_len) = rule.map(|r| r.len()) {
                                    if rule_len + 1 >= inputs_str.len() {
                                        (Vec::new(), "", Vec::new())
                                    } else {
                                        let inputs = input_tokens.collect::<Vec<_>>();
                                        let inputs_str = inputs_str[rule_len + 1..].trim_end();
                                        let inputs_templates = inputs
                                            .iter()
                                            .map(|input| Template::new(input, loc))
                                            .collect();
                            ...
                            let deps_templates =
                                deps.iter().map(|input| Template::new(input, loc)).collect();
                            prep::Edge {
                                loc,
                                target,
                                target_template,
    ...
    pub fn parse(
        ...
        content: &'a str,
        file_path: &'a str,
        escaped_indexes: &EscapedIndexes,
    ) -> Parsed<'a> {
        let mut parser = Self {
            arena,
            cursor: 0,
